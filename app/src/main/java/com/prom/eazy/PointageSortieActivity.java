...
public class PointageSortieActivity extends AppCompatActivity
    ...
    DialogBoxSuccess dialogBoxSuccess = new DialogBoxSuccess(PointageSortieActivity.this);
    DialogBoxError dialogBoxError = new DialogBoxError(PointageSortieActivity.this);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        vendeurFromIntent = (VendeurItemModel) getIntent().getSerializableExtra(EXTRA_INFO_AGENT_SORTIE);
    ...
    public void onValidateCommand(boolean facturer) {
        ...
        VendeurItemModel v = (VendeurItemModel) getIntent().getSerializableExtra(EXTRA_INFO_AGENT_SORTIE);
        ...
        if(facturer){
            ...
            if(!Printooth.INSTANCE.hasPairedPrinter()) {
                startActivityForResult(new Intent(PointageSortieActivity.this, ScanningActivity.class),ScanningActivity.SCANNING_FOR_PRINTER);
            }else{
                printText();
            }
        }
    ...
    public void fun(String username, int code_vehicule, int code_ag, int sec){
        ...
        Call<ModelIsSuccess> insertBonDeSortie = api.insertBonDeSortie(username,code_vehicule,code_ag,sec);
        insertBonDeSortie.enqueue(new Callback<ModelIsSuccess>() {
            @Override
            public void onResponse(Call<ModelIsSuccess> call, Response<ModelIsSuccess> response) {
                if (response.body().getIsSuccess() == 1) {
                    ...
                    insertBonDeSortieHashmaped(response.body().getCode_bp(),hashcall);
    ...
    public void deleteBonDeSortie(int code_bp){
        Api api = ApiAgent.getAgent().create(Api.class);
        Call<ModelIsSuccess> delete = api.deleteQteBonDeSortie(code_bp);
        ...
        insertBonDeSortieHashmaped.enqueue(new Callback<ModelIsSuccess>() {
            ...
            public void onResponse(Call<ModelIsSuccess> call, Response<ModelIsSuccess> response) {
                ...
                if (response.body().getIsSuccess() == 1) {
                    ...
                    SortieFragment.hashmap.put(new Integer(vendeurFromIntent.getCode_agent()),
                            new MyEntry<>(vendeurFromIntent, new Boolean(true)));
                }else{
                    //supprimer tous les qte de ce bon + supprimer ce bon
                    deleteBonDeSortie(code_bp);
                    //set the SortieFragment hashmap value to false so he can click him again
                    SortieFragment.hashmap.put(new Integer(vendeurFromIntent.getCode_agent()),new MyEntry<>(vendeurFromIntent,new Boolean(false)));
            ...
            @Override
            public void onFailure(Call<ModelIsSuccess> call, Throwable t) {
                Log.d("khraa","failure funhashmaped");
                //supprimer tous les qte de ce bon + supprimer ce bon
                deleteBonDeSortie(code_bp);
                //set the SortieFragment hashmap value to false so he can click him again
                SortieFragment.hashmap.put(new Integer(vendeurFromIntent.getCode_agent()),new MyEntry<>(vendeurFromIntent,new Boolean(false)));
